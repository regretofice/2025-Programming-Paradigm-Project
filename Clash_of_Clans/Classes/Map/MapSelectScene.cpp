...
bool MapSelectScene::init() {
  ...
  if (!background) {
    background =
        Sprite::create("start_background.png");  // 如果没有选择背景，用开始背景
  }
  ...
  // 标题
  auto titleLabel =
      Label::createWithSystemFont("选择地图", "Microsoft YaHei", 72);
  ...
  if (!map1Button) {
    ...
    auto map1Label =
        Label::createWithSystemFont("地图1", "Microsoft YaHei", 36);
  ...
  auto map11Name =
      Label::createWithSystemFont("攻伐模式", "Microsoft YaHei", 24);
  map11Name->setTextColor(Color4B::RED);
  map11Name->setPosition(
      Vec2(visibleSize.width / 2 - 200, visibleSize.height / 2));
  auto map111Name = Label::createWithSystemFont("在规定时间内击毁大本营",
                                                "Microsoft YaHei", 19);
  map111Name->setPosition(
      Vec2(visibleSize.width / 2 - 205, visibleSize.height / 2 - 50));
  ...
  map2Name->setPosition(
      Vec2(visibleSize.width / 2 + 200, visibleSize.height / 2 - 100));
  auto map22Name =
      Label::createWithSystemFont("建造模式", "Microsoft YaHei", 24);
  map22Name->setPosition(
      Vec2(visibleSize.width / 2 + 200, visibleSize.height / 2));
  map22Name->setTextColor(Color4B::GREEN);
  auto map222Name = Label::createWithSystemFont("请先选本模式构建大本营",
                                                "Microsoft YaHei", 19);
  map222Name->setPosition(
      Vec2(visibleSize.width / 2 + 195, visibleSize.height / 2 - 50));
  ...
  if (!returnButton) {
    ...
    auto returnLabel =
        Label::createWithSystemFont("返回", "Microsoft YaHei", 24);
