...
#include "Tools/PlacementManager.h"
...
// 调用通用退出函数
void MapScene::menuReturnCallback(Ref* pSender) { handleSceneExit(pSender); }
...
bool MapScene::init() {
  ...
  // 初始化计时器标签
  _timerLabel = Label::createWithTTF("60s", "fonts/arial.ttf", 24);
  _timerLabel->setColor(Color3B::RED);
  _timerLabel->enableOutline(Color4B::BLACK, 2);
  _timerLabel->setPosition(
      Vec2(visibleSize.width / 2, visibleSize.height - 90));  // 位于资源条下方
  this->addChild(_timerLabel, 101);
  _timerLabel->setVisible(false);  // 默认隐藏
...
void MapScene::createResourceBars() {
  ...
  // 绘制锤子形状
  // 锤子头
  builderIconDraw->drawSolidRect(Vec2(-10, -5), Vec2(10, 5),
                                 Color4F(0.3f, 0.3f, 0.3f, 1.0f));
  // 锤子柄
  builderIconDraw->drawSolidRect(Vec2(-3, -10), Vec2(3, 10),
  ...
  _builderBarForeground->drawSolidRect(
      Vec2(0, 0), Vec2(200, 20), Color4F(1.0f, 0.65f, 0.0f, 1.0f));  // 橙色前景
...
void MapScene::updateResourceDisplay() {
  ...
  float builderPercent =
      maxBuilder > 0 ? (currentBuilder * 100.0f / maxBuilder) : 0;
  ...
  std::string builderText =
      StringUtils::format("%d/%d", currentBuilder, maxBuilder);
...
void MapScene::update(float dt) {
  ...
  // 地图1的计时器逻辑
  if (_isMap1 && !_isGameOver) {
    int64_t currentTime = TimeTools::getCurrentTimeMs();
    int elapsedSeconds =
        TimeTools::getTimeDiffInSeconds(_startTimeMs, currentTime);
    _countdown = 60 - elapsedSeconds;

    if (_countdown <= 0) {
      _countdown = 0;
      _timerLabel->setString("0s");
      gameOver(false);  // 时间到，失败
    } else {
      _timerLabel->setString(StringUtils::format("%ds", _countdown));
    }
    ...
    for (auto building : buildings) {
      if (building->getBoundingBox().containsPoint(touchPos)) {
        if (UpgradeManager::getInstance()->tryUpgradeBuilding(building)) {
          ...
          this->updateResourceDisplay();  // 更新 UI 资源条
        }
        return true;
      }
    }
    ...
    if (data.buildingType == BuildingType::RESOURCE) {
      if (data.resourceType == ResourceType::GOLD) {
        building = ResourceBuilding::create(
            ...
            600 + 100 * data.level, 5.0f, ResourceType::GOLD, 10 * data.level,
            50 * data.level);
        ...
        building = ResourceBuilding::create(
            ...
            600 + 100 * data.level, 5.0f, ResourceType::ELIXIR, 10 * data.level,
            50 * data.level);
      }
      ...
      if (data.targetType == TargetType::AIR_ONLY) {
        building = DefenseBuilding::create(
            ...
            800 + 150 * data.level, 8.0f, 10 * data.level, 200,
        ...
        building = DefenseBuilding::create(
            ...
            CampType::PLAYER, data.level, 3, 200 * data.level, 50,
            800 + 150 * data.level, 8.0f, 10 * data.level, 220,
      ...
      if (data.resourceType == ResourceType::GOLD) {
        building = ResourceStorageBuilding::create(
            ...
            700 + 100 * data.level, 5.0f, ResourceType::GOLD, 1000 * data.level,
            0.5f);
      } else if (data.resourceType == ResourceType::ELIXIR) {
        building = ResourceStorageBuilding::create(
            "Elixir_Storage_0" + std::to_string(data.level) + ".png", data.name,
            CampType::PLAYER, data.level, 3, 400 * data.level, 50,
            700 + 100 * data.level, 5.0f, ResourceType::ELIXIR,
            1000 * data.level, 0.5f);
      } else {
        building = ResourceStorageBuilding::create(
            "base_camp_0" + std::to_string(data.level) + ".png", data.name,
            CampType::PLAYER, data.level, 3, 200 * data.level, 50,
            1500 + 200 * data.level, 8.0f, ResourceType::BUILDER,
            2 + data.level, 0);
      }
    }
    if (building) {
...
void MapScene::onEnter() {
  ...
  _isMap1 = (_mapFileName.find("map1") != std::string::npos);
  if (_isMap1) {
    // ：启动计时器
    _timerLabel->setVisible(true);
    _startTimeMs = TimeTools::getCurrentTimeMs();
    _countdown = 60;
    _isGameOver = false;
    _timerLabel->setString("60s");
  } else {
