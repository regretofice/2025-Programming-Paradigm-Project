...
bool MapScene::init() {
    ...
    SpriteFrameCache::getInstance()->addSpriteFramesWithFile("rarbarian_idle.plist");
...
void MapScene::createResourceBars() {
    ...
    resourcePanel->setPosition(Vec2(origin.x, origin.y + visibleSize.height - 60));
    ...
    // 绘制金币图标：黄色圆形
    goldIconDraw->drawSolidCircle(Vec2(0, 0), 15, 0, 16, 1, 1, Color4F(1.0f, 0.84f, 0.0f, 1.0f)); // 金色
    // 在圆形中间添加"$"符号
    ...
    goldBarBackground->drawSolidRect(Vec2(0, 0), Vec2(200, 20), Color4F(0.2f, 0.2f, 0.2f, 1.0f)); // 深灰色背景
    goldBarBackground->setPosition(Vec2(150, 20));
    resourcePanel->addChild(goldBarBackground);

    // 金币进度条前景
    _goldBarForeground = DrawNode::create();
    _goldBarForeground->drawSolidRect(Vec2(0, 0), Vec2(200, 20), Color4F(1.0f, 0.84f, 0.0f, 1.0f)); // 金色前景
    ...
    // 绘制圣水图标：紫色水滴
    elixirIconDraw->drawSolidCircle(Vec2(0, 0), 15, 0, 16, 1, 1, Color4F(0.58f, 0.0f, 0.83f, 1.0f)); // 紫色圆形
    ...
    elixirBarBackground->drawSolidRect(Vec2(0, 0), Vec2(200, 20), Color4F(0.2f, 0.2f, 0.2f, 1.0f)); // 深灰色背景
    ...
    _elixirBarForeground->drawSolidRect(Vec2(0, 0), Vec2(200, 20), Color4F(0.58f, 0.0f, 0.83f, 1.0f)); // 紫色前景
...
void MapScene::updateResourceDisplay() {
    ...
    _goldBarForeground->drawSolidRect(Vec2(0, 0), Vec2(goldBarWidth, 20), Color4F(1.0f, 0.84f, 0.0f, 1.0f));
    _elixirBarForeground->drawSolidRect(Vec2(0, 0), Vec2(elixirBarWidth, 20), Color4F(0.58f, 0.0f, 0.83f, 1.0f));
...
void MapScene::createPlacementMenu() {
    ...
    menuPanel->setContentSize(Size(120, visibleSize.height - 60)); // 减去资源条高度
