...
bool MapScene::init() {
    ...
    _timerLabel->setPosition(Vec2(visibleSize.width / 2, visibleSize.height - 90)); // 位于资源条下方
...
void MapScene::update(float dt) {
    ...
    if (_isMap1 && !_isGameOver) {
        ...
        int elapsedSeconds = TimeTools::getTimeDiffInSeconds(_startTimeMs, currentTime);
        ...
        if (data.buildingType == BuildingType::RESOURCE) {
            ...
            if (data.resourceType == ResourceType::GOLD) {
                ...
                building = ResourceStorageBuilding::create(
                    ...
                    CampType::PLAYER, data.level, 3, 400 * data.level, 50,
                    700 + 100 * data.level, 5.0f, ResourceType::ELIXIR, 1000 * data.level,
                    0.5f);
            }
            else {
                building = ResourceStorageBuilding::create(
                    ...
                    CampType::PLAYER, data.level, 3, 200 * data.level, 50,
                    1500 + 200 * data.level, 8.0f, ResourceType::BUILDER, 2 + data.level, 0);
            }
        }
        if (building) {
