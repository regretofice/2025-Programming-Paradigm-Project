...
ResourceStorageBuilding* ResourceStorageBuilding::create(
  ...
  ResourceStorageBuilding* storage =
      new (std::nothrow) ResourceStorageBuilding();
  if (storage && storage->init(texPath, name, camp, level, maxLevel, maxHP,
                               size, upgradeCost, upgradeTime, resType,
                               maxStorageCapacity, destroyLossRate)) {
    storage->autorelease();
    return storage;
  }
...
bool ResourceStorageBuilding::init(const std::string& texPath,
  ...
  if (!Building::init(texPath, name, camp, level, maxLevel, maxHP, size,
                      upgradeCost, upgradeTime, BuildingType::STORAGE,
                      maxStorageCapacity)) {
    return false;
  }
...
bool ResourceStorageBuilding::depositResource(int amount) {
  ...
  CCLOG("[%s]存入%d%s，当前存储：%d/%d", _name.c_str(), actualDeposit, resName,
        _currentStored, _maxStorageCapacity);
  return true;
}
...
int ResourceStorageBuilding::withdrawResource(int amount) {
  ...
  // 日志输出
  const char* resName = _resType == ResourceType::GOLD ? "金币" : "圣水";
  CCLOG("[%s]取出%d%s，剩余存储：%d/%d", _name.c_str(), actualWithdraw, resName,
        _currentStored, _maxStorageCapacity);
...
void ResourceStorageBuilding::loseResourceOnDestroy() {
  ...
  const char* resName = _resType == ResourceType::GOLD ? "金币" : "圣水";
  CCLOG("[%s]被摧毁，丢失%d%s，剩余：%d", _name.c_str(), lost, resName,
        _currentStored);
}
...
bool ResourceStorageBuilding::upgrade() {
  ...
  CCLOG("[%s]升级至%d级，存储容量提升至%d", _name.c_str(), _level,
        _maxStorageCapacity);
  return true;
}
