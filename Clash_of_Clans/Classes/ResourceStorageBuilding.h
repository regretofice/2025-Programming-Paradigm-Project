...
public:
    // Cocos2d创建函数
    static ResourceStorageBuilding* create(const std::string& texPath, const std::string& name,
...
// 便捷创建函数（语义化封装）
// 创建储金罐
inline ResourceStorageBuilding* createGoldStorage(const std::string& texPath, CampType camp, int level) {
    ...
    return ResourceStorageBuilding::create(texPath, "储金罐", camp, level, maxLevel, maxHP, size,
        upgradeCost, upgradeTime, ResourceType::GOLD, maxStorage);
}

// 创建圣水瓶
inline ResourceStorageBuilding* createElixirStorage(const std::string& texPath, CampType camp, int level) {
    int maxLevel = 3;
    ...
    return ResourceStorageBuilding::create(texPath, "圣水瓶", camp, level, maxLevel, maxHP, size,
        upgradeCost, upgradeTime, ResourceType::ELIXIR, maxStorage);
}
