#ifndef __AIR_BALLONSOLDIER_H__
#define __AIR_BALLONSOLDIER_H__

#include "Soldier.h"

class BallonSoldier : public Soldier {
 public:
  static BallonSoldier* create(int hp, int attack, int attack_range,
                               int attack_CD);
  ...
  SoldierType getSoldierType() const override {
    return SoldierType::kAirBallonSoldier;
  }
  ...
  ~BallonSoldier() override;  // 子类析构不再释放动画（全局缓存统一管理）
 protected:
  BallonSoldier() = default;
  ...
  const int EXPLODE_DAMAGE = 30;     // 爆炸伤害
};

#endif  // __AIR_BALLONSOLDIER_H__