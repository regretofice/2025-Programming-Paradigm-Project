...
class Building : public Sprite {
 protected:
  ...
  // 血条相关
  DrawNode* _hpBarBackground;  // 血条背景
  DrawNode* _hpBarForeground;  // 血条前景
  bool _showHpBar;             // 是否显示血条
  float _hpBarWidth;           // 血条宽度
  float _hpBarHeight;          // 血条高度
  float _hpBarOffsetY;         // 血条Y轴偏移
 ...
 public:
  ...
  // 血条相关方法
  void createHpBar();         // 创建血条
  void updateHpBar();         // 更新血条显示
  void showHpBar(bool show);  // 显示/隐藏血条
  ...
  int getUpGradeCost() const { return _upgradeCost; }
