#ifndef __GROUND_GIANT_H__
#define __GROUND_GIANT_H__

#include "Soldier.h"

class Giant : public Soldier {
 public:
  static Giant* create(int hp, int attack, int attack_range, int attack_CD);
  ...
  SoldierType getSoldierType() const override {
    return SoldierType::kGroundGiant;
  }
  ...
  ~Giant() override;  // 子类析构不再释放动画（全局缓存统一管理）
 protected:
  Giant() = default;
