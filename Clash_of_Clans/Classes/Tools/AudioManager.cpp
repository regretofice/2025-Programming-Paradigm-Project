...
void AudioManager::preloadAllAudio() {
  ...
  std::vector<std::string> audioFiles = {
      "start_music.mp3", "combat_music.mp3", "home_music.mp3",
      "victory.mp3",  // 胜利音效
      "defeat.mp3"    // 失败音效
                      // 可以根据需要添加更多音频文件
  };
...
void AudioManager::stopSoundEffect(int soundId) {
  if (soundId != -1) {
    ...
    // 从活动列表中移除
    auto it =
        std::find(_activeSoundIds.begin(), _activeSoundIds.end(), soundId);
...
int AudioManager::playSoundInternal(const std::string& filePath, bool loop,
                                    float volume) {
  // 检查文件是否存在
  ...
  if (soundId != AudioEngine::INVALID_AUDIO_ID) {
    ...
    AudioEngine::setFinishCallback(
        soundId, [this, soundId](int id, const std::string& filePath) {
          // 从活动列表中移除
          auto it =
              std::find(_activeSoundIds.begin(), _activeSoundIds.end(), id);
          if (it != _activeSoundIds.end()) {
            _activeSoundIds.erase(it);
          }
        });
