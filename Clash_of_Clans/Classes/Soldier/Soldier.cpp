...
#include "Tools/SoldierTargetPreference.h"
Soldier::Soldier()
...
void Soldier::recalculatePathTo(Building* target) {
  ...
  int sx = std::max(
      0, std::min(static_cast<int>(startTile.x), pm->getMapWidth() - 1));
  int sy = std::max(
      0, std::min(static_cast<int>(startTile.y), pm->getMapHeight() - 1));
  int gx = std::max(
      0, std::min(static_cast<int>(goalTile.x), pm->getMapWidth() - 1));
  int gy = std::max(
      0, std::min(static_cast<int>(goalTile.y), pm->getMapHeight() - 1));
  ...
  if (getSoldierMoveType() == SoldierMoveType::kGround) {
    ...
    CCLOG(" BuildingManager found %d buildings, soldier r=%.1f",
          static_cast<int>(buildings.size()), myRadius);
    for (auto building : buildings) {
      ...
      int bx = static_cast<int>(bTilePos.x);
      int by = static_cast<int>(bTilePos.y);
...
void Soldier::moveToNextPathPoint() {
  ...
  if (!target_building_ || target_building_->isDestroyed()) {
    ...
    this->changeState(SoldierState::kIdle);  // 切换状态
    this->startAttack();
    return;
  }
...
void Soldier::update(float dt) {
  ...
  if (current_state_ == SoldierState::kIdle && !isDead()) {
    // 尝试每隔一段时间（1秒）重新寻找目标，防止发呆
    static float searchTimer = 0;
    searchTimer += dt;
    if (searchTimer > 1.0f) {
      this->startAttack();
      searchTimer = 0;
