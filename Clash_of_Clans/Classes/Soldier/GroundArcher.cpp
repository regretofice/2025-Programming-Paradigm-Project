#include "Soldier/GroundArcher.h"
...
Archer* Archer::create(int hp, int attack, int attackrange, int attackCD) {
  Archer* ret = new (std::nothrow) Archer();
...
bool Archer::init(int hp, int attack, int attack_range, int attack_CD) {
  if (!Soldier::init("archer_icon.png", hp, attack, attack_range, attack_CD)) {
    return false;
  }

  // 初始化碰撞半径
  createPhysicsBody(30.0f);
  // 设置移动速度
  setSpeed(120.0f);
  // 加载动画配置
  loadAllAnimations();
...
void Archer::playAttackAnimation() {
  auto anim =
...
void Archer::attackSoldier(Soldier* target) {
  if (!target) return;
